...
class WeatherFragmentAdapter : RecyclerView.Adapter<WeatherFragmentAdapter.WeatherViewHolder>() {
    ...
    inner class WeatherViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(dayWeatherForecast: DayWeatherForecast) {
            itemView.apply {
                findViewById<TextView>(R.id.week_day).text = dayWeatherForecast.day.dayName
                findViewById<TextView>(R.id.morning_forecast).text = dayWeatherForecast.timeForecasts[0].toString()
                findViewById<TextView>(R.id.noon_forecast).text = dayWeatherForecast.timeForecasts[1].toString()
                findViewById<TextView>(R.id.evening_forecast).text = dayWeatherForecast.timeForecasts[2].toString()
                findViewById<TextView>(R.id.night_forecast).text = dayWeatherForecast.timeForecasts[3].toString()
            }
        }
    }
}